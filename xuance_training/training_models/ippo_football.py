...
def parse_args():
    parser = argparse.ArgumentParser(
        "IPPO of XuanCe for Google Football Research environments."
    )
    parser.add_argument("--method", type=str, default="ippo")
    parser.add_argument("--env", type=str, default="football")
    parser.add_argument("--env-id", type=str, default="11_vs_11_easy_stochastic")
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=f"./ippo_football_configs/{parser.parse_args().env_id}.yaml",
    )
...
class Runner:
    def __init__(self, args):
        ...
        folder_name = f"seed_{args.seed}_" + time.asctime().replace(" ", "").replace(
            ":", "_"
        )
        self.args.model_dir_load = self.args.model_dir
        self.args.model_dir_save = os.path.join(
            os.getcwd(), self.args.model_dir, folder_name
        )
        ...
        self.envs.num_agents = self.num_agents
        self.num_adversaries = self.envs.num_adversaries
        self.dim_obs, self.dim_act, self.dim_state = (
            self.envs.dim_obs,
            self.envs.dim_act,
            self.envs.dim_state,
        )
    ...
    def get_actions(
        self, obs_n, avail_actions, *rnn_hidden, state=None, test_mode=False
    ):
        rnn_hidden_policy, rnn_hidden_critic = rnn_hidden[0], rnn_hidden[1]
        rnn_hidden_next, actions_n, log_pi_n = self.agents.act(
            obs_n, *rnn_hidden_policy, avail_actions=avail_actions, test_mode=test_mode
        )
        if test_mode:
            rnn_hidden_critic_next, values_n = None, 0
        else:
            rnn_hidden_critic_next, values_n = self.agents.values(
                obs_n, *rnn_hidden_critic, state=state
            )
        return {
            "actions_n": actions_n,
            "log_pi": log_pi_n,
            ...
            "rnn_hidden_critic": rnn_hidden_critic_next,
        }

    def get_battles_info(self):
        battles_game, battles_won = (
            self.envs.battles_game.sum(),
            self.envs.battles_won.sum(),
        )
    ...
    def get_battles_result(self, last_battles_info):
        ...
        win_rate = (
            incre_battles_won / incre_battles_game if incre_battles_game > 0 else 0.0
        )
        return win_rate
    ...
    def run_episodes(self, test_mode=False):
        ...
        rnn_hidden = self.agents.policy.representation.init_hidden(
            self.n_envs * self.num_agents
        )
        rnn_hidden_critic = self.agents.policy.representation_critic.init_hidden(
            self.n_envs * self.num_agents
        )

        while not envs_done.all():  # start episodes
            available_actions = self.envs.get_avail_actions()
            actions_dict = self.get_actions(
                obs_n,
                available_actions,
                rnn_hidden,
                rnn_hidden_critic,
            ...
            next_obs_n, next_state, rewards, terminated, truncated, info = (
                self.envs.step(actions_dict["actions_n"])
            )
            envs_done = self.envs.buf_done
            rnn_hidden, rnn_hidden_critic = (
                actions_dict["rnn_hidden"],
                actions_dict["rnn_hidden_critic"],
            )
            ...
            if test_mode:
                ...
                for i_env in range(self.n_envs):
                    ...
                    if terminated[i_env] or truncated[i_env]:  # one env is terminal
                        ...
                        if terminated[i_env]:
                            values_next = np.array(
                                [0.0 for _ in range(self.num_agents)]
                            )
                        else:
                            batch_select = np.arange(
                                i_env * self.num_agents, (i_env + 1) * self.num_agents
                            )
                            kwargs = {"state": [next_state[i_env]]}
                            rnn_h_critic_i = self.agents.policy.representation_critic.get_hidden_item(
                                batch_select, *rnn_hidden_critic
                            )
                            _, values_next = self.agents.values(
                                next_obs_n[i_env : i_env + 1], *rnn_h_critic_i, **kwargs
                            )
                        self.agents.memory.finish_path(
                            i_env,
                            self.env_step + 1,
                            *terminal_data,
    ...
    def test_episodes(self, test_T, n_test_runs):
        ...
        results_info = {
            "Test-Results/Mean-Episode-Rewards": mean_test_score,
    ...
    def run(self):
        if self.args.test_mode:
            ...
            test_score_mean, test_score_std, test_win_rate = self.test_episodes(
                0, n_test_episodes
            )
            agent_info = f"Algo: {self.args.agent}, Map: {self.args.env_id}, seed: {self.args.seed}, "
            print(
                agent_info,
                "Win rate: %.3f, Mean score: %.2f. " % (test_win_rate, test_score_mean),
            )
            ...
            while self.current_step <= self.running_steps:
                ...
                if (self.current_step - last_test_T) / test_interval >= 1.0:
                    ...
                    print(
                        agent_info,
                        "Win rate: %.3f, Mean score: %.2f. "
                        % (train_win_rate, np.mean(episode_scores)),
                        time_pass,
                        time_left,
                    )
                    episode_scores = []
    ...
    def time_estimate(self, start):
        ...
        time_left = int(
            (self.running_steps - self.current_step) / self.current_step * time_pass
        )
        if time_left < 0:
            time_left = 0
        hours_pass, hours_left = time_pass // 3600, time_left // 3600
        min_pass, min_left = (
            np.mod(time_pass, 3600) // 60,
            np.mod(time_left, 3600) // 60,
        )
        sec_pass, sec_left = np.mod(np.mod(time_pass, 3600), 60), np.mod(
            np.mod(time_left, 3600), 60
        )
