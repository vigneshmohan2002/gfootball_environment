...
def parse_args():
    parser = argparse.ArgumentParser(
        "VDN of XuanCe for Google Football Research environments."
    )
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=f"./vdn_football_configs/{parser.parse_args().env_id}.yaml",
    )
...
class Runner:
    ...
    def get_actions(self, obs_n, avail_actions, rnn_hidden, test_mode=False):
        rnn_hidden_next, actions_n = self.agents.act(
            obs_n, *rnn_hidden, avail_actions=avail_actions, test_mode=test_mode
        )
    ...
    def run_episodes(self, test_mode=False):
        ...
        while not envs_done.all():  # start episodes
            ...
            actions_dict = self.get_actions(
                obs_n, available_actions, rnn_hidden, test_mode=test_mode
            )
                ...
                for i_env in range(self.n_envs):
                    ...
                    if terminated[i_env] or truncated[i_env]:  # one env is terminal
                        ...
                        self.agents.memory.finish_path(
                            i_env, self.env_step + 1, *terminal_data
                        )
                        self.current_step += 1
                self.env_step += 1
